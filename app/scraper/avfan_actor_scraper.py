...
class AvfanActorScraper:
    ...
    def open_listing_page(self, page, actor_name, page_number):
        url = self.build_listing_url(actor_name, page_number)
    ...
    @staticmethod
    def build_listing_url(actor_name, page_number):
        safe_actor_name = quote(str(actor_name or '').strip())
        return f'https://avfan.com/search?q={safe_actor_name}&st=cast&page={int(page_number)}'
