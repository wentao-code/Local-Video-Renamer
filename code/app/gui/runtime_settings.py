...
from app.core.project_paths import BACKGROUND_REFRESH_SETTINGS_FILE, RUNTIME_SETTINGS_FILE
...
def load_background_refresh_enabled(settings_file=BACKGROUND_REFRESH_SETTINGS_FILE):
    path = Path(settings_file)
    if not path.exists():
        return True
    try:
        ...
        return True
    return bool((payload or {}).get('enabled', True))


def save_background_refresh_enabled(
    enabled=True,
    settings_file=BACKGROUND_REFRESH_SETTINGS_FILE,
):
    path = Path(settings_file)
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = json.dumps({'enabled': bool(enabled)}, ensure_ascii=False, indent=2)
    fd, temp_name = tempfile.mkstemp(prefix=f'{path.name}.', suffix='.tmp', dir=str(path.parent))
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as handle:
            handle.write(payload)
