...
from app.core.project_paths import BROWSER_PROFILES_DIR, LAYOUT_ROOT, PROJECT_ROOT
...
def get_path_setting(key, default_relative_path):
    ...
    if not path.is_absolute():
        path = (LAYOUT_ROOT if path.parts and path.parts[0] in {'user_data', 'runtime'} else PROJECT_ROOT) / path
    return path
...
def get_browser_profiles_dir():
    value = _get_text_setting('BROWSER_PROFILES_DIR', default='')
    if not value or Path(value) == Path('browser_profiles'):
        return BROWSER_PROFILES_DIR
    return get_path_setting('BROWSER_PROFILES_DIR', value)


def get_avfan_profile_dir():
    value = _get_text_setting('AVFAN_PROFILE_DIR', default='')
    if value and Path(value) != Path('browser_profiles') / 'avfan':
        path = Path(value)
        if not path.is_absolute():
            path = (LAYOUT_ROOT if path.parts and path.parts[0] in {'user_data', 'runtime'} else PROJECT_ROOT) / path
        return path
