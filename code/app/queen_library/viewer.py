...
class QueenLibraryWindow(AsyncTaskHostMixin, QDialog):
    ...
    def showEvent(self, event):
        ...
        if not self.is_async_task_running():
            self.poll_crawl_progress()

    def closeEvent(self, event):
        if self._is_backend_crawl_running():
            self.hide()
    ...
    def _is_backend_crawl_running(self):
        try:
            progress = dict(self.backend_client.get_queen_refresh_progress() or {})
        except Exception:
            progress = {}
