...
def build_merged_movie_snapshot(code, library_rows, processed_row=None, cache_row=None):
    ...
    return {
        ...
        'author': _pick_best_author(candidates),
        'author_raw': _pick_best_author_raw(candidates, _pick_best_author(candidates)),
        ...
        'javtxt_enrichment_status': _status_from_search_state(
            best_search_state,
...
def _build_processed_candidate(processed_row=None, cache_row=None):
    ...
    return {
        ...
        'author': normalize_second_source_actor_text(cache_row.get('javtxt_actors', processed_row.get('author', ''))),
        ...
        'javtxt_release_date': str(cache_row.get('javtxt_release_date', processed_row.get('javtxt_release_date', '')) or '').strip(),
        'avfan_url': '',
...
def _status_from_search_state(search_state, fallback_status=UNENRICHED_STATUS):
    ...
    if search_state == JAVTXT_SEARCH_STATE_NO_RESULT:
        ...
        return normalized_fallback if is_no_result_status(normalized_fallback) else NO_SEARCH_RESULTS_STATUS
...
def _author_quality(author, author_raw):
    if normalize_second_source_actor_text(author):
        return 2
    if normalize_actor_raw_text(author_raw):
        return 1
    return 0
...
def _is_better_title(candidate, current, code):
    ...
    if not str(current or '').strip():
        return True
    return _title_score(candidate, code) > _title_score(current, code)


def _title_score(title, code):
    normalized_title = str(title or '').strip()
    return (0 if normalized_title.upper() == str(code or '').strip().upper() else 1, len(normalized_title))


def _is_better_text(candidate, current):
    candidate_text = str(candidate or '').strip()
    current_text = str(current or '').strip()
    return bool(candidate_text) and (not current_text or len(candidate_text) > len(current_text))
