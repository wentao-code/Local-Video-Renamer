...
from app.services.identity import normalize_actor_name, split_actor_names
from app.services.library import ActorProfileUpdateService, CodePrefixLibrary, extract_code_prefix
...
class LibraryAdminService:
    ...
    def rename_actor(self, old_name, new_name, birthday='', age=''):
        ...
        payload = self.actor_profile_update_service.normalize_payload(new_name, birthday=birthday, age=age)
        normalized_new_name = payload['name']
        ...
        if normalized_old_name != normalized_new_name and any(
            str(enrichment_record.get(field, '')).strip()
            for field in ('actor_id', 'enrichment_status', 'last_error', 'last_enriched_at')
        ):
            raise ValueError(f'演员 {normalized_new_name} 已存在补全记录')
        if normalized_old_name != normalized_new_name and (
            int(enrichment_record.get('avfan_total_pages', 0) or 0) > 0
            or int(enrichment_record.get('avfan_total_videos', 0) or 0) > 0
        ):
            raise ValueError(f'演员 {normalized_new_name} 已存在补全记录')
