...
- 新增 `login_status_service.py`，专门负责：
  - 进入主页后点击右上角用户图标
  - 判断当前是“已登录”还是“未登录”
  - 未登录时自动跳转登录页并填充 `user_data/config/.env` 中的账号密码
  - 等待用户手动完成图片验证码和最终登录
...
- `user_data/config/.env` 保存本地网页访问配置，也不应提交到 Git。
- `user_data/browser_profiles/` 保存网页登录状态、Cookie 等个人敏感数据，也不应提交到 Git。
...
- 提供“自动登录”按钮，用于打开登录页并自动填入本地 `user_data/config/.env` 中保存的账号密码。
...
重构后，外层目录将桌面代码、移动端工程、持久化用户数据与运行产物分开：
...
│     ├─ path_library.py
│     └─ video_enrichment.py
├─ mobile_app/                 # 独立 Flutter APP 工程，纳入 Git
├─ user_data/                  # 本地持久化数据，不提交
│  ├─ config/                  # .env 与界面设置
│  ├─ databases/               # SQLite 数据库
│  ├─ browser_profiles/        # 登录状态与 Cookie
│  ├─ backups/                 # 本地备份
│  └─ snapshots/               # 持久化刷新快照
└─ runtime/                    # 可再生运行产物，不提交
   ├─ logs/
...
- `启动系统.bat`
  ...
  - 会优先读取本地 `user_data/config/.env` 中的 `APP_PYTHON_EXE`
...
- 自动填入账号密码。
- 等待用户手动完成图片验证码并点击登录。
- 登录成功后复用同一套 `user_data/browser_profiles/avfan` 登录状态。
...
- 使用 `user_data/browser_profiles/avfan` 作为 Playwright 专用持久化登录档案，复用登录 Cookie 和本地存储。
...
- 提供重置函数清理 `user_data/browser_profiles/avfan`，用于验证状态或 Cookie 卡死时重新建立网页登录环境。
- 处理成年确认弹窗。
- 在首页搜索框输入视频编号并点击搜索。
- 打开搜索结果详情页。
...
用户点击“重置网页登录”
  ...
  -> 删除 user_data/browser_profiles/avfan
```
...
用户点击“自动登录”
  ...
  -> 打开登录链接并自动填入账号密码
  -> 用户手动输入图片验证码并点击登录
  -> 登录成功后复用 user_data/browser_profiles/avfan
```
...
- `user_data/config/.env` 中保存的是本机网页访问配置，仓库中只保留 `.env.example` 作为模板。
- `user_data/browser_profiles/` 中可能保存网页登录状态、Cookie、LocalStorage 等敏感信息，只允许本机使用。
